...
#include <fcntl.h>
//#include <termios.h>
#include <asm/termios.h> // Error about incomplete struct for "if (ioctl(serial, TCSETS2, &tio) < 0)" with standard <termios.h>
#include <stdio.h>
#include <argp.h>
...
static struct argp_option options[] = 
{
	...
	{"megaeverdrivepro", 'm', 0  , 0, "Enable Mega Everdrive Pro serial header mode" },
	{ 0 }
};
...
void arg_set_defaults(arguments_t *arguments)
{
	...
	arguments->nonstandard  = -1; // Scott Baker's 31250 MIDI BAUD addition
...
void write_midi_action_to_serial_port(snd_seq_t* seq_handle) 
{
	...
	do 
	{
		...
		if(ev == NULL) { // solve audacious' time-shifting null output
		        bytes[0] == 0x00;
		        bytes[1] == 0x00;
		        bytes[2] == 0xFF;
		        break;
		...
		if (bytes[0]!=0x00)
		{
			...
			if (bytes[2]==0xFF) {
			        if (arguments.megaeverdrivepro) {
				        char megaproheader[13] = {0x2B, 0xD4, 0x1A, 0xE5, 0x01, 0x81, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x00};
					char megaprolen[4] = {0xFF, 0xFF, 0xFF, 0xFF};
				        megaprolen[0] = (2 >> 24) & 0xFF;
					megaprolen[1] = (2 >> 16) & 0xFF;
					megaprolen[2] = (2 >> 8) & 0xFF;
					megaprolen[3] = (2) & 0xFF;
					megaproheader[8] = megaprolen[0];
					megaproheader[9] = megaprolen[1];
					megaproheader[10] = megaprolen[2];
					megaproheader[11] = megaprolen[3];
					write(serial, megaproheader, 13);
				}
				...
				write(serial, bytes, 2);
			        ...
			        if (arguments.megaeverdrivepro) {
				        char megaproheader[13] = {0x2B, 0xD4, 0x1A, 0xE5, 0x01, 0x81, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x00};
					char megaprolen[4] = {0xFF, 0xFF, 0xFF, 0xFF};
				        megaprolen[0] = (3 >> 24) & 0xFF;
					megaprolen[1] = (3 >> 16) & 0xFF;
					megaprolen[2] = (3 >> 8) & 0xFF;
					megaprolen[3] = (3) & 0xFF;
					megaproheader[8] = megaprolen[0];
					megaproheader[9] = megaprolen[1];
					megaproheader[10] = megaprolen[2];
			...
			if (!arguments.silent && arguments.verbose) {
			        char *sysex = ev->data.ext.ptr;
			        printf("Alsa    0x%x System exclusive  ", sysex[0]&0xFF);
				int i;
				for (i=0; i<ev->data.ext.len; i++) {
				        printf(" %03u", sysex[i]&0xFF);
				}
				printf("\n");
				sysex = NULL;
			}
			if (arguments.megaeverdrivepro) {
			        char megaproheader[13] = {0x2B, 0xD4, 0x1A, 0xE5, 0x01, 0x81, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x00};
				char megaprolen[4] = {0xFF, 0xFF, 0xFF, 0xFF};
				megaprolen[0] = (ev->data.ext.len >> 24) & 0xFF;
				megaprolen[1] = (ev->data.ext.len >> 16) & 0xFF;
				megaprolen[2] = (ev->data.ext.len >> 8) & 0xFF;
				megaprolen[3] = (ev->data.ext.len) & 0xFF;
				megaproheader[8] = megaprolen[0];
				megaproheader[9] = megaprolen[1];
				megaproheader[10] = megaprolen[2];
				megaproheader[11] = megaprolen[3];
			...
			write(serial, ev->data.ext.ptr, ev->data.ext.len);
		}
...
int main(int argc, char** argv)
{
	...
	/* 
	 * Add support for standard 31250 MIDI BAUD, but a nonstandard serial rate.
	 */
	if (arguments.nonstandard > 0) {
	        struct termios2 tio;
		if (ioctl(serial, TCGETS2, &tio) < 0)
		        return -1;
		tio.c_cflag &= ~CBAUD;
		tio.c_cflag |= BOTHER;
		tio.c_ispeed = arguments.nonstandard;
		tio.c_ospeed = arguments.nonstandard;
		if (ioctl(serial, TCSETS2, &tio) < 0)
		        return -1;
	}
